...
for (par_ind, paragraph) in par.items():
	...
	for (perm_ind, permutation) in enumerate(random.sample(a, 6)):
		...
		for (ind, val) in enumerate(dep):
			...
			nodes[val["id"]] = word + "-" + str(ind) + "-" + str(perm_ind)
		for (ind, val) in enumerate(dep):
			word = nlp_perm.text[val["start"]:val["end"]]
		...
		for (ind_node, node_) in enumerate(nodes):
			try:
				pass#nodes_mat[ind_node] = glove_model[node_.split("-")[0]]
			except:
				pass		
		...
		graphs.append((nodes_mat, edge_mat, label/12.0))

	pickle.dump(edge_types, open("edge_types_no_conceptnet.pcl", "wb"))
	pickle.dump(graphs, open("graphs_no_conceptnet.pcl", "wb"))