...
with open("edges_lists2.pcl", "rb") as f:
    edge_data = pickle.load(f, encoding='latin1') 

with open("node_mats2.pcl", "rb") as f:
    labels = pickle.load(f, encoding='latin1') 

labels = [i.astype(int) for i in labels]

edge_src = [[i[0] for i in edge_dat] for edge_dat in edge_data]
edge_dst = [[i[1] for i in edge_dat] for edge_dat in edge_data]
edge_types = [[i[2] for i in edge_dat] for edge_dat in edge_data]
num_classes = int(max(list(labels[0]))) + 1

num_nodes = [max(max(edge_src[i]), max(edge_dst[i])) + 1 for i in range(len(edge_src))]
num_rels = len(edge_types_list)

edge_types = [torch.from_numpy(np.array(edge_type)).float() for edge_type in edge_types]
edge_norms = [torch.from_numpy(np.zeros(len(edge_s))).unsqueeze(1).float() for edge_s in edge_src]
...
for i in range(len(edge_src)):
    ...
    g.edata.update({'rel_type': np.array(edge_types[i]).astype("long"), 'norm': edge_norms[i]})
    gs.append(g)
...
# optimizer
optimizer = torch.optim.Adam(model.parameters(), lr=0.1)#, weight_decay=l2norm)
...
for epoch in range(2500):
    ...
    print(ind)
    l = labels[ind]
    idx = [k for k in list(range(num_nodes[ind])) if l[k] in [0,2,3]]
    train_idx = random.sample(idx, len(idx) - 20)
    val_idx = [k for k in idx if k not in train_idx]
    optimizer.zero_grad()
    print("pre")
    logits = model.forward(gs[ind])
    print("post")
    #print((logits[train_idx].shape, logits[train_idx][0]))
    loss = F.nll_loss(logits[train_idx], l[train_idx])
    loss.backward()

    optimizer.step()

    train_acc = torch.sum(logits[train_idx].argmax(dim=1) == l[train_idx])
    train_acc = train_acc.item() / len(train_idx)
    val_loss = F.nll_loss(logits[val_idx], l[val_idx])
    val_acc = torch.sum(logits[val_idx].argmax(dim=1) == l[val_idx])
