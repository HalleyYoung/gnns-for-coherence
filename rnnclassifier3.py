...
for (par_ind, paragraph) in list(par.items()):
    print(par_ind)
    a = 0#random.randint(1, len(paragraph) - 2)
...
class RNN(nn.Module):
    ...
    def forward(self, x, y):
        ...
        c_out1 = c_out1.permute(1,0,2).contiguous().view(32,-1)
        c_out2 = c_out2.permute(1,0,2).contiguous().view(32,-1)
        c_out = torch.stack((c_out1, c_out2), dim=1).view(32,-1)
        #, 1)
        #print(h_out.shape)
        ...
        return self.sigmoid(self.out2(self.out1(c_out))).view((32,2))
