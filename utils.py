...
def generate_sampled_graph_and_labels(triplets, sample_size, split_size,
    ...
    src = [i[0] for i in triplets]
    rel = [i[1] for i in triplets]
    dst = [i[2] for i in triplets]
    #edges = triplets[edges]
    src, rel, dst = (np.array([src[edge] for edge in edges]), np.array([rel[edge] for edge in edges]), np.array([dst[edge] for edge in edges]))#edges.transpose()
